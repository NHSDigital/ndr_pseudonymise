...
class Issue4810Test < ActiveSupport::TestCase
  test 'has_a_version_number' do
    refute_nil ::NdrPseudonymise::VERSION
  end

  TESTDIR = File.join('test', 'unit')
  SAMPLE_DATA = File.join(TESTDIR, 'sample_prescription_data.csv')
  SAMPLE_SPEC = File.join(TESTDIR, 'prescription_data_format_0.0.1.yml')
  # SAMPLE_SALT = File.join(TESTDIR, 'sample_salt.yml')    # not needed here
  SAMPLE_SALT_BUNDLE = File.join(TESTDIR, 'salt_4810.kek')
  SAMPLE_RSA_PUB = File.join(TESTDIR, 'sample4810_rsa3072_pub.pem')
  SAMPLE_RSA_PRIV = File.join(TESTDIR, 'sample4810_rsa3072.pem')
  ...
  test 'core_demographics' do
    assert_equal([[%w(pat_nhs_num 9999999492)],
                  [%w(pat_dob 1911-02-12), %w(pat_address_postcode PO331AA)]],
                 @pspec.core_demographics(@row1))
  end

  test 'real_ids and pseudo_id' do
    real_ids = @pspec.real_ids(@row1)
    assert_equal('pat__nhs__num_9999999492', real_ids[0])
    assert_equal('pat__dob_pat__address__postcode_1911-02-12_PO331AA', real_ids[1])
  ...
  test 'all_demographics' do
    ...
    assert_equal(@row1[0..7], all_demog)
  ...
  test 'clinical_data' do
    ...
    assert_equal(@row1[8..-1], all_demog)
    all_demog.each_with_index do |x, i|
      # KH: convert nil to ''
      x = x.nil? ? '' : x
      assert(x.is_a?(String), "Expected column #{i} with value #{x.inspect} to be a String")
